...
**2. API Configuration:**
Create a `.env` file in the root directory. The system is designed to seamlessly auto-detect your provider based on the key you supply, making it incredibly easy for reviewers to test.

To ensure a frictionless review process without credit card requirements, you can use a free Groq API key (OpenAI compatible):
```env
# .env
...
* **Page-Level Context Summarization (Signal vs. Noise):** Financial PDFs are full of noise (indices, disclaimers). Before extracting facts, the Extractor LLM is forced to generate a brief "Page Analysis" summarizing what the page means. Facts are extracted against this context, reducing hallucinations.
...
* **Real-Time UI Logging:** To solve the UX issue of Streamlit "hanging" during long processes, the UI features real-time terminal-style logging, allowing the user to watch the chunking, extracting, and reasoning agents work in real-time.
